...
async def new_game(user: User, response: Response):
    res = OrderedDict()
    async with httpx.AsyncClient() as client:
    # Retrieving game information
        r = await client.put('http://localhost:9999/start/', json={"username": user.username})
        res.update(r.json())
        ...
        if res["status"] == "in-progress":
            ...
            r = await client.put('http://localhost:9999/check/', json={"word": "words"})
            ...
            for word in guesses:
                r = await client.put('http://localhost:9999/check/', json={"word": word})
                data = r.json()
                if data["correct"]:
